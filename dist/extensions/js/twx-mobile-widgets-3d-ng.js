...
/* globals Module, VF_ANG, $, ThingView, escape, angular, THREE, _ */
...
(function () {
  ...
  function renderer($rootScope, $timeout, thingViewPath) {
      ...
      if (!session) {
        ...
        if (els && els.length) {
          ...
          if (window.builderSettings && window.builderSettings.enableDebugLogging) {
            addRenderStat(session, 'ion-view');
          }
        }
      }
    }
      ...
      if (obj.GetType() === 'Model') {
        ...
        for (var obj2 in vrSession) {
          if (vrSession[obj2].GetType() === 'Model Item') {
            ...
            updateRuntimeModelTargetLocation(widget, vrSession[obj2]);
          }
        }
      }
    ...
    function updateRuntimeModelTargetLocation(model, modelTarget) {
      var modelLocation = model.GetLocation();
      var modelTargetWidget = modelTarget.GetWidget();
      ...
      modelTarget.rotation = {
          rx: modelLocation.orientation.x,
          ry: modelLocation.orientation.y,
          rz: modelLocation.orientation.z
      };
      modelTarget.translation = {
          x: modelLocation.position.x,
          y: modelLocation.position.y,
          z: modelLocation.position.z
      };
    }
    ...
    this.addPVS = function (name, id, url, cull, parent, successCb, errorCb) {
      ...
      // Check widget does not already exist
      ...
      if (sessionObj) {
        ...
        if (widget) {
          let errorMessage = createErrorMessage(
            ...
            "Could not add model: " + id + "; node already exists"
          );
      ...
      if (!session) {
        let errorMessage = createErrorMessage(
          ...
          "Could not load model: " + id + "; TV session is not initialized"
        );
        setTimeout(() => errorCb(errorMessage));
        return null;
      }
      ...
      widget.LoadModel = function(url, successCb, errorCb) {
        ...
        widget.LoadFromURLWithCallback(url, true, false, false, function(success, isStructure, errors) {
    ...
    this.addModelItem = function (name) {
      ...
      if (arr[0].endsWith("-") && arr[0].length > 1) {
        ...
        if (session) {
          ...
          // Apply Scale Function
          widget.ApplyScale = function (obj) {
            let scale = obj.scale ? obj.scale.sx : undefined;
            if (!isNaN(scale)) {
              widget.SetScale(Number(scale));
            }
          };
        ...
        if(src.startsWith("vuforia-model://")) {
          ...
          $rootScope.$on('loaded3DObj', function (evt, data) {
            ...
            if(data && data.name === id) {
              ...
              self.setTranslation(id, undefined, floor.pos.y, undefined);
            }
          });
    ...
    this.add3DObject = function(name, id, vertices, normals, texcoords, indexes, color, texture, parent, successCb, failureCb) {
      setTimeout(successCb, 0);
    };
      ...
      controller: function ($scope, $element, $http) {
        ...
        ctrl.cvApiReady = function () {
          ...
          if ($rootScope.builderSettings.enableDebugLogging) {
            addRenderStat(ctrl.mySession, 'twx-dt-view');
          }
    ...
    if (views && views.length) {
      ...
      setInterval(function() {
        session.GetRenderStats(function(rs) {
          ...
          updateRenderStat(canvas, text, "16px Georgia", 20);
        });
      }, 1000);
    }
  }

  function updateRenderStat(canvas, text, font, height) {
    var context = canvas.getContext('2d');
